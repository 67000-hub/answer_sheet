...
基于 Python + OpenCV 的答题卡自动识别与判分系统，支持**单选、4选项、多列布局**的答题卡，并支持**主观题 OCR 文字识别**和**模板批量处理**。
...
1. **选择图片**：弹出文件选择对话框
2. **输入答案**：填写每题正确答案（逗号或空格分隔），如 `B,D,A,C`
3. **选择是否包含主观题**：
   - **否**：自动检测全图答题卡
   - **是**：
     1. 输入主观题数量
...
│   ├── roi_selector.py   # ROI 框选工具（鼠标拖拽，支持多区域）
...
│   └── template.py       # 答题卡模板管理（创建/加载/保存/列表）
...
| `utils/roi_selector.py` | `ROISelector` 鼠标拖拽框选 ROI 工具，支持多区域选择 |
